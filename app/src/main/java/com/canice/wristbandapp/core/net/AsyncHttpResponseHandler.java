...
public class AsyncHttpResponseHandler { //请求返回处理  成功 失败 开始  完成  等自定义的消息
    ...
    protected static final int SUCCESSFORIS_MESSAGE = 4;

    private Handler handler;
    private boolean isForis = false;
    ...
    public void setForis(boolean isForis) {
		this.isForis = isForis;
	}
    ...
    protected void sendSuccessMessage(int statusCode, Header[] headers, byte[] responseBody) {
        sendMessage(obtainMessage(SUCCESSFORIS_MESSAGE, new Object[]{Integer.valueOf(statusCode), headers, responseBody}));
    }
    ...
    protected void handleSuccessMessage(int statusCode, Header[] headers, byte[] bytes) {
    ...
    void sendResponseMessage(HttpResponse response) {
        ...
        if(status.getStatusCode() >= 300) {
        	...
        	if(isForis){
        		try {
        			sendSuccessMessage(status.getStatusCode(), response.getAllHeaders(), EntityUtils.toByteArray(entity));
        		} catch (IOException e) {
